/**
 * \file tests/test_graph_properties_2strand_faces.cpp
 * \brief Unit tests for graph properties 2-strand faces.
 */
...
int main()
{
    ...
    g.render_knot();
    assert(g.properties()->face_count() == baseline_faces + 1);
    assert(g.properties()->face_degree_distribution().value(2, 0) == baseline_deg2 + 1);
