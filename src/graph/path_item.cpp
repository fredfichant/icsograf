...
Line::Line(QPointF begin, QPointF end) : begin(begin), end(end) {}
...
Compound::Compound(Line* l) : Line(*l) { append(l); }
...
Compound::~Compound()
{
    foreach (Line* l, elements) delete l;
}
...
{
    if (qFuzzyCompare(a->begin, b->end) || qFuzzyCompare(a->end, b->begin)) {
        return true;
