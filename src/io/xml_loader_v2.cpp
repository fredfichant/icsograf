...
bool XML_Loader_v2::load(QIODevice* input)
{
    ...
    if (knot.isNull()) return false;  // XML does not contain a knot
    version = knot.attribute("version").toInt();
    if (version > supported_version) return false;  // unknown version
...
void XML_Loader_v2::get_graph(Graph& kv)
{
    if (enter("style")) {
        ...
        // Removed border related code

        Node_Style ns = get_cusp("cusp");
        ns.enabled_style = Node_Style::EVERYTHING;
        if (!ns.cusp_shape) ns.cusp_shape = resource_manager().default_cusp_shape();
        ...
        if (!e_cusp.isNull()) {
            ...
            if (!e_gap.isNull())
                kv.default_edge_style_reference().crossing_distance = e_gap.text().toDouble();
        }
...
QPen XML_Loader_v2::get_pen(QString name, QPen pen)
{
    ...
    pen_col.setAlpha(pen_elem.attribute("alpha", QString::number(pen.color().alpha())).toInt());
...
void XML_Loader_v2::leave() { current_elements.pop(); }
