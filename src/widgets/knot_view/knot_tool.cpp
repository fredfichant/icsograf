...
Knot_Tool::Knot_Tool(Knot_View* view, Graph* graph) : view(view), graph(graph), m_enabled(false) {}
...
Node* Knot_Tool::node_at(QPointF pos) const { return view->node_at(pos); }

Edge* Knot_Tool::edge_at(QPointF pos) const { return view->edge_at(pos); }
    ...
    if (event.event->button() == Qt::LeftButton) {
        ...
        if (node) {
            if (modifier) {
                ...
                if (!node->isSelected()) view->scene()->clearSelection();
            ...
            if (modifier) {
                ...
                if (!edge->isSelected()) view->scene()->clearSelection();
...
bool Edge_Chain_Tool::press(const Mouse_Event& event)
{
    if (event.event->button() == Qt::LeftButton) {
        ...
        if (!last_node || next_node != last_node) {
            ...
            if (!next_node) next_node = view->add_breaking_node(event.snapped_pos);
...
{
    if (last_node) guide.setLine(QLineF(last_node->pos(), event.snapped_pos));

    notify_pos = event.snapped_pos;
}

QCursor Edge_Chain_Tool::cursor() const { return Qt::CrossCursor; }
...
void Edge_Chain_Tool::deactivate() { break_chain(); }

void Edge_Chain_Tool::break_chain()
{
    if (guide.scene()) view->scene()->removeItem(&guide);
    last_node = nullptr;
}

Toggle_Edge_Tool::Toggle_Edge_Tool(Knot_View* view, Graph* graph) : Knot_Tool(view, graph)
{
    start_node = nullptr;
    guide.setPen(QPen(Qt::gray, 0, Qt::DashLine));
}
...
void Toggle_Edge_Tool::move(const Mouse_Event& event, QPointF& notify_pos)
{
    ...
    if (start_node) guide.setLine(QLineF(start_node->pos(), event.mouse_pos));
}
