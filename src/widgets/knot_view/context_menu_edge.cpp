...
void Context_Menu_Edge::popup(Edge* e, QPoint pos)
{
    ...
    action_reset_style->setEnabled(edge->style().enabled_style != Edge_Style::EDGE_TYPE ||
                                   edge->style().edge_type !=
                                       resource_manager().default_edge_type());

    menu_edge_types->clear();
    for (Edge_Type* es : resource_manager().edge_types()) {
...
{
    view->push_command(
        new Change_Edge_Type(edge, edge->style().edge_type,
                             resource_manager().edge_type_from_machine_name(type_name), view));
}

void Context_Menu_Edge::remove() { view->remove_edge(edge); }
...
void Context_Menu_Edge::break_intersections()
{
    ...
    for (QGraphicsItem* ietm : edge->collidingItems()) {
    ...
    for (Edge* o : intersected) {
        QLineF intersect_line = o->to_line();
        QPointF intersect_point;

#if (QT_VERSION >= QT_VERSION_CHECK(5, 14, 0))
        start_line.intersects(intersect_line, &intersect_point);
#else
    ...
    std::sort(new_nodes.begin(), new_nodes.end(), Node_Comparator(edge->vertex1()));
...
void Context_Menu_Edge::subdivide()
{
    bool ok;
    int segments =
        QInputDialog::getInt(view, tr("Break edge"), tr("Number of segments"), 2, 2, 32, 1, &ok);
    if (ok) {
