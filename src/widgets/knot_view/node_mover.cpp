/**
 * \file src/widgets/knot_view/node_mover.cpp
 * \brief Implementation of node mover interactions in the knot view.
 */
...
Node_Mover::Node_Mover() : scale_factor(1), scale_count(0), rotate_angle(0), dragged_handle(nullptr)
{
...
void Node_Mover::set_nodes(QList<Node*> nodes)
{
    ...
    QPointF min, max;
    if (!m_nodes.empty()) min = max = m_nodes[0]->pos();
    ...
    if (!m_nodes.empty()) initialize_movement(pivot);  // m_nodes[0]->pos());
...
{
    for (int i = 0; i < n_handles; i++) scene->addItem(&transform_handles[i]);
}
    ...
    if (anchor_angle) {
        for (int i = 0; i < n_handles; i++)
            if (&transform_handles[i] == dragged_handle) {
                switch (i) {
                    case 0:
                        sp = m_initial_box.bottomRight();
                        break;
                    case 1:
                        sp = m_initial_box.topRight();
                        break;
                    case 2:
                        sp = m_initial_box.topLeft();
                        break;
                    case 3:
                        sp = m_initial_box.bottomLeft();
                        break;
                }
                break;
            }
...
void Node_Mover::drag_handle(QPointF p, bool fixed, double step_size)
{
    ...
    if (mode() == Transform_Handle::ROTATE) {
        ...
        if (fixed) angle = int(angle) / 15 * 15;
        rotate(angle);
...
QCursor Node_Mover::current_handle_cursor()
{
    if (dragged_handle && mode() == Transform_Handle::SCALE) {
        if (dragged_handle == transform_handles + 0 || dragged_handle == transform_handles + 2)
...
void Node_Mover::initialize_movement_internal(QPointF pivot)
{
    ...
    for (Node* n : m_nodes) offset.push_back(n->pos() - pivot);
}
...
void Node_Mover::set_pos(QPointF p) { move(p - pivot); }
...
void Node_Mover::fixed_scale(bool increase, double step_size)
{
    ...
    double factor = (m_initial_box.width() + scale_count * step_size) / m_initial_box.width();
    scale_factor = factor;
    for (int i = 0; i < m_nodes.size(); i++) {
        QLineF ray(QPointF(0, 0), offset[i]);
        ray.setLength(ray.length() * scale_factor);
