...
Dock_Knot_Style::Dock_Knot_Style(QWidget* parent) : QDockWidget(parent)
{
    setupUi(this);
    label_tooltip();

    combo_cusp_shape->clear();
    for (Cusp_Shape* cs : resource_manager().cusp_shapes())
        combo_cusp_shape->addItem(cs->icon(), cs->name(), QVariant::fromValue(cs));
    ...
    connect(Resource_Manager::pointer(), SIGNAL(cusp_shapes_changed()), SLOT(reload_cusp_shapes()));
}
...
void Dock_Knot_Style::set_style(const Node_Style& nst, const Edge_Style& est)
{
    ...
    spin_spacing->setValue(est.spacing);
    spin_strand_count->setValue(est.strand_count);
}

Node_Style Dock_Knot_Style::node_style() const
{
    return Node_Style(spin_cusp_angle->value(), spin_handle_length->value(),
                      spin_cusp_distance->value(), cusp_shape(), Node_Style::EVERYTHING);
}

Edge_Style Dock_Knot_Style::edge_style() const
{
    return Edge_Style(spin_handle_length->value(), spin_crossing_gap->value(),
                      slide_edge_slide->value() / 100.0, resource_manager().default_edge_type(),
                      Edge_Style::EVERYTHING, spin_spacing->value(), spin_strand_count->value());
}
...
void Dock_Knot_Style::reload_cusp_shapes()
{
    ...
    for (int i = 0; i < resource_manager().cusp_shapes().size(); i++) {
        ...
        if (cs == current_shape) current_index = i;
        combo_cusp_shape->addItem(cs->icon(), cs->name(), QVariant::fromValue(cs));
    }

    if (current_index >= combo_cusp_shape->count()) current_index = 0;
    combo_cusp_shape->setCurrentIndex(current_index);
    blockSignals(false);

    Cusp_Shape* new_shape = cusp_shape(current_index);

    if (new_shape != current_shape) emit cusp_shape_changed(new_shape);
}

void Dock_Knot_Style::emit_edge_slide(int percent) { emit edge_slide_changed(percent / 100.0); }

void Dock_Knot_Style::refresh_data() { reload_cusp_shapes(); }
